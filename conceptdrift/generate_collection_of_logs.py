...
def generate_logs(num_logs=50, num_traces=1000, drifts=['sudden', 'gradual', 'recurring', 'incremental'], drift_area=[0.2, 0.8], proportion_random_evolution=[0.2, 0.6], noise=[0, 0.1], tree_one=generate_specific_trees('middle'), filepath=str(pathlib.Path().resolve())):
    ...
    :param proportion_random_evolution: possible proportion of activities to be affected by the random change as a range in list
    :param tree_one: process model as process tree
    ...
    with open(filepath+'/gold_standard/gold_standard.csv', 'w', newline='') as log_file:
        ...
        for i in range(num_logs):
            ...
            if len(proportion_random_evolution) == 1:
                ran_evolve = round(proportion_random_evolution[0], 2)
            else:
                ran_evolve = round(uniform(proportion_random_evolution[0], proportion_random_evolution[1]), 2)
            ...
            if drift == 'sudden':
                event_log = sudden_drift(num_traces, drift_area_one, tree_one, tree_two)
                ...
                event_log = gradual_drift(num_traces, drift_area_one, drift_area_two, gr_type, tree_one, tree_two)
                ...
                if drift_area_one > 0 and drift_area_two != 1:
                    ...
                    sea_cha = randint(1, 6)
                    dr_s = str(sea_cha)+" seasonal changes"
                event_log = recurring_drift(num_traces, drift_area_one, drift_area_two, sea_cha, pro_first, tree_one, tree_two)
            ...
            file_object.close()
    ptml_exporter.apply(tree_one, filepath+"/gold_standard/initial_version.ptml")
